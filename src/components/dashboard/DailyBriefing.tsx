...
export function DailyBriefing() {
  ...
  const priorityClients = clients.filter((c: Client) =>
        ...
        <div className="p-5">
          ...
          {summary.topAlerts.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {summary.topAlerts.slice(0, 3).map((alert: Alert) => (
...
function AlertRow({ alert, clients }: { alert: Alert; clients: Client[] }) {
  const client = clients.find((c: Client) => c.id === alert.clientId);
