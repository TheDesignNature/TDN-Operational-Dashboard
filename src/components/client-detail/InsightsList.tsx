...
// Suggested actions are static per client — in production these would
// come from OpenClaw as part of the Insight payload
const SUGGESTED_ACTIONS: Record<string, string[]> = {
  ...
  "foundation-home": [
    ...
    "Audit daily budget caps — ensure they're not too low for monthly targets",
...
export function InsightsList({ clientId }: InsightsListProps) {
  ...
  useEffect(() => {
    getInsightsForClient(clientId).then((data) => {
      setInsights(data);
      setLoading(false);
    });
  }, [clientId]);
